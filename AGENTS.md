...
**Key files**:
- `src/content.config.ts` — the `projects` collection schema (must live at this exact path, not `src/content/config.ts` — Astro 7 requires it at the project root of `src/`)
...
- `src/pages/projects/[id].astro` — case-study template (note: `id`, not `slug` — Astro 5+ deprecated `entry.slug` in favor of `entry.id`, and legacy `type: 'content'` collections are no longer supported without a loader)
