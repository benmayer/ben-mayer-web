    ...
    <div class="feed flex flex-col w-full max-w-xl m-auto">
        <ul v-if="!loading" class="text-left my-8">
          <li v-for="post of blogs" :key="post.id" class="feed-item shadow hover:shadow-sm">
            <NuxtLink :to="{ name: 'blog-id', params: { 'id': post.id }}" class="flex flex-col items-stretch h-48 sm:flex-row">
              <div class="w-full sm:w-1/4 mr-2 bg-gray-600 bg-opacity-10 bg-cover bg-center" :style="{backgroundImage: `url('${post.teaserImageUrl}')`}">
              </div>
        ...
        <ul v-else class="w-full text-left my-8">
          <li class="feed-item items-stretch flex shadow hover:shadow-sm">
              <div class="w-full sm:w-1/4 mr-2 bg-gray-600 bg-opacity-10 bg-cover bg-center"></div>
